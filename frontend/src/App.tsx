...
export default function App() {
    ...
    try {
      ...
      if (budRes.status === 'fulfilled') {
        ...
        if (data && data.length > 0) {
          ...
          // Restore user's persisted savings goal
          const savedGoalItem = data.find((item) => item.savings_goal && item.savings_goal > 0);
          if (savedGoalItem && savedGoalItem.savings_goal) {
            setSavingsGoal(savedGoalItem.savings_goal);
          }
        } else {
          setBudget({
            totalLimit: 0,
  ...
  // Dynamic calculations - Filtered for valid non-outlier transactions
  ...
  const handleAddTransaction = async (newTxData: Omit<Transaction, 'id'>) => {
    if (currentUser) {
      try {
        ...
        if (err?.status === 409) {
          showToast('Duplicate transaction detected (submitted within last 10 seconds)', 'warning');
        } else {
          showToast('Failed to save — check your connection', 'warning');
        }
        return;
  ...
  const handleAddRecurring = async (itemData: Omit<RecurringItem, 'id'>) => {
    if (currentUser) {
      try {
        const created = await apiFetch<RecurringItem>('/recurring', {
          method: 'POST',
          body: JSON.stringify({
            merchant: itemData.name,
            name: itemData.name,
            amount: itemData.amount,
            frequency: itemData.frequency || 'monthly',
            date: itemData.date,
            next_due_date: itemData.next_expected_date || itemData.dueDate,
            icon: itemData.icon || 'subscriptions',
            interval_days: itemData.interval_days || 30,
          }),
        });
        setRecurring((prev) => [created, ...prev]);
        showToast(`Recurring subscription "${itemData.name}" added!`, 'success');
        return;
      } catch (err) {
        console.warn('Failed to add recurring item:', err);
        showToast('Failed to save — check your connection', 'warning');
      }
    }
  ...
  const handleDeleteRecurring = async (id: string) => {
    const itemToDelete = recurring.find((i) => i.id === id);
    if (currentUser && !id.startsWith('rec-')) {
      try {
        ...
        console.warn('Failed to delete recurring item:', err);
        showToast('Failed to delete — check your connection', 'warning');
      }
    }
    setRecurring((prev) => prev.filter((i) => i.id !== id));
    showToast(
    ...
    return (
      <div className="min-h-screen bg-[#051424] text-[#d4e4fa] flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <span className="material-symbols-outlined text-3xl animate-spin text-[#3b82f6]">sync</span>
          <p className="text-xs font-mono-data text-[#c7c4d8]">Loading SpendWise...</p>
        </div>
      </div>
    );
  }
  ...
  if (!currentUser) {
    return (
      <LoginPage
        onLoginSuccess={() => {
          // auth state will update currentUser and render dashboard
        }}
      />
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#051424] text-[#d4e4fa] flex flex-col font-sans selection:bg-[#3b82f6]/40">
      ...
      <Modals
        ...
        savingsGoal={savingsGoal}
        onSaveSavingsGoal={async (newGoal) => {
          ...
          if (currentUser) {
            try {
              await apiFetch('/budgets', {
                method: 'POST',
                body: JSON.stringify({
                  monthly_limit: budget.totalLimit,
