        ...
        <div className="space-y-2 max-h-[175px] overflow-y-auto pr-1 custom-scrollbar">
          {recurringItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-6 text-center">
              <div className="w-10 h-10 rounded-xl bg-[#1c2b3c]/60 flex items-center justify-center text-[#93c5fd]/50 mb-2 border border-white/5">
                <span className="material-symbols-outlined text-xl">event_repeat</span>
              </div>
              <p className="text-xs font-semibold text-[#c7c4d8]/80">No recurring items</p>
              <button
                onClick={onManageRecurring}
                className="text-[11px] text-[#93c5fd] hover:text-white font-semibold mt-1 underline cursor-pointer font-mono-data"
              >
                + Add subscription
              </button>
            </div>
            ...
            recurringItems.map((item, idx) => {
              ...
              const statusColor = isDueSoon ? 'text-amber-300 bg-amber-400/10 border-amber-400/20' : 'text-[#93c5fd] bg-[#3b82f6]/10 border-[#3b82f6]/30';
