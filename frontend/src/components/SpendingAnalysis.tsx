...
export const SpendingAnalysis: React.FC<SpendingAnalysisProps> = ({ transactions }) => {
  ...
  const getDataset = (): { items: IncomeExpenseBarData[]; totalActivity: number } => {
    ...
    if (timeframe === 'yearly') {
      ...
      labels = Array.from({ length: 6 }, (_, i) => String(currentYear - 5 + i));
    } else {
    ...
    const rawData = labels.map((lbl, mIdx) => {
      let labelIncome = 0;
      let labelExpense = 0;
  ...
  return (
    <div className="col-span-12 lg:col-span-8 bento-card bg-[#122131] p-6 animate-fade flex flex-col justify-between">
      ...
      <div className="h-64 flex items-end justify-between gap-1 sm:gap-3 px-1 sm:px-3 pb-4 border-b border-[#464555]/10 relative">
        ...
        {totalActivity === 0 ? (
          ...
          <>
            ...
            {/* Bars */}
            {data.map((item, index) => (
              <div key={item.label + timeframe + index} className="flex-1 h-full flex flex-col justify-end items-center group min-w-0">
                <div className="flex items-end gap-1 w-full justify-center h-full">
                  ...
                  {item.incomeHeight > 0 && (
                    <div
                      ...
                      className="w-2.5 sm:w-3.5 rounded-t-sm bar-animate bar-hover bg-[#60a5fa] shadow-[0_0_12px_rgba(96,165,250,0.35)] cursor-pointer"
                    ></div>
                  )}
                  ...
                  {item.expenseHeight > 0 && (
                    <div
                      ...
                      className="w-2.5 sm:w-3.5 rounded-t-sm bar-animate bar-hover bg-[#1d4ed8] border-t border-[#93c5fd]/40 shadow-[0_0_12px_rgba(29,78,216,0.35)] cursor-pointer"
                    ></div>
                  )}
                </div>
                <p className="font-mono-data text-[9px] sm:text-[10px] mt-2.5 text-[#c7c4d8]/70 font-medium truncate">
                  {item.label}
                </p>
              </div>
            ))}
      ...
      <div className="flex justify-center gap-8 mt-6">
        <div className="flex items-center gap-2.5 cursor-pointer hover:opacity-100 transition-opacity">
          <div className="w-3 h-3 rounded-full bg-[#60a5fa] shadow-[0_0_6px_#60a5fa]"></div>
          <span className="font-mono-data text-xs text-[#d4e4fa] font-semibold">Income (+)</span>
        </div>
        <div className="flex items-center gap-2.5 cursor-pointer hover:opacity-100 transition-opacity">
          <div className="w-3 h-3 rounded-full bg-[#1d4ed8] border border-[#93c5fd]/50 shadow-[0_0_6px_#1d4ed8]"></div>
