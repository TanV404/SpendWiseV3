...
export const Modals: React.FC<ModalsProps> = ({
  ...
  const [emailAddress, setEmailAddress] = useState('alex.rivera@example.com');
  ...
  const isValidDateString = (dateStr: string): { valid: boolean; formatted: string } => {
    ...
    if (isoMatch) {
      ...
      if (!isNaN(d.getTime())) {
        return { valid: true, formatted: d.toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }) };
      }
    }
  ...
  const handleCSVImport = async () => {
    ...
    try {
      const res = await apiFetch<{ created: Transaction[]; errors: { row: number; reason: string }[] }>(
        ...
        {
          method: 'POST',
          body: JSON.stringify({ raw_csv: csvText }),
        }
      );
      ...
      if (res.created && res.created.length > 0) {
        ...
        // Fallback commit validated rows directly if local
      ...
      setCsvStatus(`Successfully imported ${createdCount} transaction(s). ${errorCount > 0 ? `${errorCount} row(s) skipped.` : ''}`);
      setTimeout(() => {
        onClose();
        setCsvStatus(null);
        setCsvText('');
      ...
      <div className="modal-content max-h-[90vh] overflow-y-auto custom-scrollbar" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex justify-between items-center mb-6 pb-2 border-b border-[#464555]/20">
          <h2 className="font-bold text-lg text-[#bfdbfe] tracking-tight flex items-center gap-2">
            ...
            {activeModal === 'notifications' && 'Notifications & Email Alerts'}
          </h2>
          <button
            onClick={onClose}
...
Netflix Premium, Entertainment, -15.99, May 01 2024`}
            ...
            {csvValidation && !csvValidation.valid && (
              <div className="p-3.5 bg-red-900/30 border border-red-500/50 rounded-xl text-xs text-[#ffb4ab]">
              ...
              <div className="space-y-3">
                <div className="p-3 bg-[#122131] border border-[#464555]/30 rounded-xl flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="px-2.5 py-1 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-lg text-xs font-mono-data font-bold">
                      {csvValidation.readyRows.length} Ready to Import
                    </span>
                    {csvValidation.skippedRows.length > 0 && (
                      <span className="px-2.5 py-1 bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded-lg text-xs font-mono-data font-bold">
                ...
                {/* Skipped Rows List */}
                {csvValidation.skippedRows.length > 0 && (
                  <div className="p-3 bg-amber-950/30 border border-amber-500/30 rounded-xl max-h-32 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="space-y-1">
                      {csvValidation.skippedRows.map((err, idx) => (
                        <div key={idx} className="text-xs text-[#c7c4d8] font-mono-data flex items-start gap-2">
                          <span className="text-amber-400 font-bold">Row {err.row}:</span>
          ...
          <form onSubmit={handleTransactionSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex justify-end gap-3 pt-3 border-t border-[#464555]/20">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 rounded-xl text-xs font-semibold text-[#c7c4d8] hover:bg-[#1c2b3c]"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="px-5 py-2 bg-[#3b82f6] text-white rounded-xl text-xs font-bold hover:bg-[#3b82f6]/90 shadow-md"
              >
        ...
        {activeModal === 'notifications' && (
          <div className="space-y-5">
            ...
            <div className="space-y-2.5">
              <div className="flex justify-between items-center">
                <h4 className="text-xs font-bold text-[#bfdbfe] uppercase tracking-wider font-mono-data">
                  Recent Alerts
                </h4>
                <button
                  onClick={() => {
                    setEmailAlertToast('Notifications cleared');
                    setTimeout(() => setEmailAlertToast(null), 2000);
                  }}
                  className="text-[10px] text-[#c7c4d8]/70 hover:text-white transition-colors"
                >
                  Clear all
                </button>
              </div>

              <div className="p-3 bg-[#122131] rounded-xl border border-[#3b82f6]/30 flex items-start gap-3">
                <span className="material-symbols-outlined text-[#ffb4ab] text-lg mt-0.5">warning</span>
                <div>
                  <p className="text-xs font-bold text-[#d4e4fa]">Dining Out Limit Warning</p>
                  <p className="text-[11px] text-[#c7c4d8]">You are at 85% of your weekly cafe allowance.</p>
                  <span className="text-[9px] text-[#c7c4d8]/50 font-mono-data">10 mins ago</span>
                </div>
              </div>

              <div className="p-3 bg-[#122131] rounded-xl border border-[#464555]/20 flex items-start gap-3">
                <span className="material-symbols-outlined text-emerald-400 text-lg mt-0.5">payments</span>
                <div>
                  <p className="text-xs font-bold text-[#d4e4fa]">Income Credited</p>
                  <p className="text-[11px] text-[#c7c4d8]">Transaction recorded from recent deposit.</p>
                  <span className="text-[9px] text-[#c7c4d8]/50 font-mono-data">Yesterday</span>
                </div>
              </div>
            </div>
            ...
            <button
              onClick={onClose}
              className="w-full py-2.5 bg-[#1c2b3c] text-[#d4e4fa] hover:bg-[#2c3a4c] rounded-xl text-xs font-semibold transition-colors cursor-pointer"
            >
              Close Notifications
            </button>
          </div>
